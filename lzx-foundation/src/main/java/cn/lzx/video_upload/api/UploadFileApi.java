package cn.lzx.video_upload.api;

import cn.lzx.video_upload.utils.FormatDateUtils;
import cn.lzx.video_upload.utils.JsonUtils;
import cn.lzx.video_upload.utils.RandomStrUtils;
...
public class UploadFileApi {
    ...
    public String fileUpload(@RequestParam("file") MultipartFile file, HttpServletRequest request) {
        return this.uploadFileUtils(file, request, "/files",
     ...
     * @return 上传请求执行结果
     */
    private String uploadFileUtils(MultipartFile file, HttpServletRequest request,
                                   String saveChildPath, String fileType) {
        // 上传文件大小（默认为配置为图片）
        ...
        try {
            if (!file.isEmpty()) {
                ...
                // 重命名
                fileName = FormatDateUtils.getDateStr(new Date(),
                        "yyyyMMddHHmmss") + RandomStrUtils.createRandomStr(3) + extName;
                // 获取上传文件MIME-type类型
                ...
                if (fileType.contains(contentType)) {
                    ...
                    if (file.getSize() > (uploadObjectSize * 1024 * 1024)) {
                        ...
                        // 0表示成功，1失败
                        mapMsg.put("code", 1);
                        mapMsg.put("msg", "上传失败，可上传文件大小不超过" +
                                uploadObjectSize + "MB（" +
                                (uploadObjectSize * 1024 * 1024) + "字节）。");
